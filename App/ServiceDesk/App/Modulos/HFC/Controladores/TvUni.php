	...
	class TvUni extends Controlador {
		...
		public function Index() {
			...
			$plantilla->Parametro('listaAveria', $this->Modelo->listaAveria());
 			...
 			echo $plantilla->MostrarPlantilla('TvUni', 'Index.html');
		}
		
		/**
		 * TvUni::IndexValidacionFormulario()
		 * 
		...
		private function IndexSelectDependiente() {
			return NeuralJQueryScript::IdPrincipal('ubicacion')
				...
				->URL(NeuralRutasApp::RutaUrlAppModulo('HFC', 'TvUni', 'ubicacionAjax'))
				->Puntero('UBICACION')
		...
		public function ajaxJs($archivo = false) {
			...
			$plantilla = new NeuralPlantillasTwig(APP);
			echo $plantilla->MostrarPlantilla(implode(DIRECTORY_SEPARATOR, array('TvUni', 'ajaxJs.js')));
		}
		
		
		/**
		 * TvUni::Horas & HorasCalculo
		 * 
		...
		public function Horas($archivo = false) {
			...
			AppSesion::validar('ESCRITURA');
			$plantilla = new NeuralPlantillasTwig(APP);
			echo $plantilla->MostrarPlantilla(implode(DIRECTORY_SEPARATOR, array('TvUni', 'Horas.js')));
		}
		...
		private function ubicacionAjaxMostrar() {
			...
			echo $plantilla->MostrarPlantilla('TvUni', 'ubicacionAjaxMostrar', $archivo);
		}
		
		/**
		 * TvUni::ubicacionAjaxSeleccion()
		 *
		...
		private function ajaxProcesoPlantilla($array = false) {
			$plantilla = $this->Modelo->consultaPlantilla('HFC', 'TVUNIDIRECCIONAL', 'PRIORIDAD '.$array['PRIORIDAD']);
			...
			return $guion->MostrarPlantilla('TvUni', 'ajaxProcesoPlantilla.html');
		}
		
		/**
		 * TvUni::ajaxPlantillaBase()
		 * 
		...
		private function ajaxPlantillaBase($guion = false, $ubicacion = false) {
			...
			echo $plantilla->MostrarPlantilla('TvUni', 'ajaxPlantillaBase.html');
		}
	}