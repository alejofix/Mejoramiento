	...
	class TvBi extends Controlador {
		...
		public function Index() {
			...
			$plantilla->Parametro('listaAveria', $this->Modelo->listaAveria());
 			...
 			echo $plantilla->MostrarPlantilla('TvBi', 'Index.html');
		}
		
		/**
		 * TvBi::IndexValidacionFormulario()
		 * 
		...
		private function IndexSelectDependiente() {
			return NeuralJQueryScript::IdPrincipal('ubicacion')
				...
				->URL(NeuralRutasApp::RutaUrlAppModulo('HFC', 'TvBi', 'ubicacionAjax'))
				->Puntero('UBICACION')
		...
		public function ajaxJs($archivo = false) {
			...
			$plantilla = new NeuralPlantillasTwig(APP);
			echo $plantilla->MostrarPlantilla(implode(DIRECTORY_SEPARATOR, array('TvBi', 'ajaxJs.js')));
		}
		
		
		/**
		 * TvBi::Horas & HorasCalculo
		 * 
		...
		public function Horas($archivo = false) {
			...
			AppSesion::validar('ESCRITURA');
			$plantilla = new NeuralPlantillasTwig(APP);
			echo $plantilla->MostrarPlantilla(implode(DIRECTORY_SEPARATOR, array('TvBi', 'Horas.js')));
		}
		...
		private function ubicacionAjaxMostrar() {
			...
			echo $plantilla->MostrarPlantilla('TvBi', 'ubicacionAjaxMostrar', $archivo);
		}
		
		/**
		 * TvBi::ubicacionAjaxSeleccion()
		 *
		...
		private function ajaxProcesoPlantilla($array = false) {
			$plantilla = $this->Modelo->consultaPlantilla('HFC', 'TVBIDIRECCIONAL', 'PRIORIDAD '.$array['PRIORIDAD']);
			...
			return $guion->MostrarPlantilla('TvBi', 'ajaxProcesoPlantilla.html');
		}
		
		/**
		 * TvBi::ajaxPlantillaBase()
		 * 
		...
		private function ajaxPlantillaBase($guion = false, $ubicacion = false) {
			...
			echo $plantilla->MostrarPlantilla('TvBi', 'ajaxPlantillaBase.html');
		}
	}